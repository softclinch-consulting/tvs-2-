...
export const GateAppContainer: React.FC = () => {
  ...
  const handleSimulateScan18 = () => {
    // Scan the first 18 tyres belonging to the selected job for fast demonstration.
    currentTyres.slice(0, Math.min(18, currentTyres.length)).forEach((tyre) => {
      gateScanPhysicalTyre(currentJob.id, tyre.id);
    });
  ...
  const handleCompleteReceivingClick = () => {
    ...
    if (scannedTyres.length < (currentJob.confirmedQty || currentJob.pickupQty)) {
          ...
          {gateScreen === 10 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-white">Log Evidence Case {selectedCase?.id || 'Pending'}</h3>
              </div>
              ...
              <div className="space-y-2 text-xs">
                ...
                <button
                  onClick={() => {
                    if (selectedCase) submitMismatchEvidence(selectedCase.id, [], evidenceRemarks);
                    setGateScreen(11);
                  }}
            ...
            <div className="space-y-4 text-center pt-2">
              ...
              <div className="bg-slate-800 p-3 rounded-xl border border-slate-700 text-left text-xs space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-amber-400">{selectedCase?.id || 'Pending'}</span>
                </div>
              </div>
              ...
              <button
                onClick={() => {
                  setActiveApp('management');
                  setMgmtScreen(9);
                  if (selectedCase) setSelectedCaseId(selectedCase.id);
                }}
                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-xl text-xs"
              >
