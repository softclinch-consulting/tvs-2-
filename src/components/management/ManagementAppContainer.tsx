...
export const ManagementAppContainer: React.FC = () => {
  ...
  const [selectedComplaintId, setSelectedComplaintId] = useState('CMP-2026-0042');
  ...
  const handleAdvanceProduction = () => {
    ...
    const advanced = advanceProductionStage(currentJob.id, selectedProdStage, mat);
    if (!advanced) return;
    const stages: ProcessingStage[] = ['BUFFING', 'RASPING', 'REPAIR', 'COATING', 'CURING', 'PCI', 'PDI'];
    const nextStage = stages[stages.indexOf(selectedProdStage) + 1];
    if (nextStage) {
      setSelectedProdStage(nextStage);
    } else {
      setMgmtScreen(19);
    }
  };
  ...
  const handleQcSubmit = () => {
    ...
    if (submitQcApproval(currentJob.id, qc)) {
      setQcFeedback('QC approved and saved. Opening Order Status Timeline.');
      setMgmtScreen(20);
      return;
    }

    const pendingTyres = tyres.filter((t) => t.jobCardId === currentJob.id && t.status !== 'MISSING' && t.fitnessStatus === 'PENDING').length;
    const openMismatch = mismatchCases.some((c) => c.jobCardId === currentJob.id && c.status !== 'RESOLVED');
    if (currentJob.processingStage !== 'PDI') {
      setQcFeedback(`QC is locked until PDI is completed. Current production stage: ${currentJob.processingStage || 'Not started'}.`);
    } else if (pendingTyres > 0) {
      setQcFeedback(`QC is locked: ${pendingTyres} tyre(s) still need inspection classification.`);
    } else if (openMismatch) {
      setQcFeedback('QC is locked until the open mismatch case is resolved in Mismatch Center.');
    } else {
      setQcFeedback('QC could not be saved. Check the selected Job Card and current role.');
    }
  };
        ...
        {mgmtScreen === 19 && (
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 space-y-4">
              ...
              {qcFeedback && (
                <p className={`text-xs font-semibold ${qcFeedback.startsWith('QC approved') ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {qcFeedback}
                </p>
              )}
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                {currentJob.status === 'READY_FOR_DISPATCH' && (
                  <button
                    onClick={() => {
                      if (dispatchOrder(currentJob.id)) setMgmtScreen(20);
                    }}
                ...
                {currentJob.status === 'DISPATCHED' && (
                  <button
                    onClick={() => {
                      if (deliverOrder(currentJob.id)) setMgmtScreen(20);
                    }}
