...
interface AppContextType {
  ...
  advanceProductionStage: (jobCardId: string, stage: ProcessingStage, materialUsage?: MaterialUsage) => boolean;
  submitQcApproval: (jobCardId: string, qcRecord: QCApprovalRecord) => boolean;
  ...
  dispatchOrder: (jobCardId: string) => boolean;
  deliverOrder: (jobCardId: string) => boolean;
...
const STORAGE_KEY = 'tvs-tread-workflow-state';

const readPersisted = <T,>(key: string, fallback: T): T => {
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    const state = raw ? JSON.parse(raw) as Record<string, unknown> : undefined;
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [jobCards, setJobCards] = useState<JobCard[]>(() => readPersisted('jobCards', INITIAL_JOB_CARDS));
  const [tyres, setTyres] = useState<Tyre[]>(() => readPersisted('tyres', INITIAL_TYRES));
  const [mismatchCases, setMismatchCases] = useState<MismatchCase[]>(() => readPersisted('mismatchCases', INITIAL_MISMATCH_CASES));
  const [auditLogs, setAuditLogs] = useState<AuditLog[]>(() => readPersisted('auditLogs', INITIAL_AUDIT_LOGS));
  const [internalMessages, setInternalMessages] = useState<InternalMessage[]>(() => readPersisted('internalMessages', INITIAL_INTERNAL_MESSAGES));
  const [whatsAppMessages, setWhatsAppMessages] = useState<WhatsAppMessage[]>(() => readPersisted('whatsAppMessages', INITIAL_WHATSAPP_MESSAGES));
  const [tyreCompanies, setTyreCompanies] = useState<TyreCompany[]>(() => readPersisted('tyreCompanies', INITIAL_TYRE_COMPANIES));
  const [users] = useState<UserProfile[]>(MOCK_USERS);
  const [customers, setCustomers] = useState<CustomerProfile[]>(() => readPersisted('customers', INITIAL_CUSTOMERS));
  const [retreadOrders, setRetreadOrders] = useState<RetreadOrder[]>(() => readPersisted('retreadOrders', INITIAL_RETREAD_ORDERS));
  const [complaints, setComplaints] = useState<Complaint[]>(() => readPersisted('complaints', INITIAL_COMPLAINTS));
  const [payments, setPayments] = useState<PaymentTransaction[]>(() => readPersisted('payments', INITIAL_PAYMENTS));
  const [sapExchangePayload, setSapExchangePayload] = useState<SapB1ExchangePayload>(() => readPersisted('sapExchangePayload', INITIAL_SAP_EXCHANGE));
  ...
  useEffect(() => {
    ...
    const workflowState = {
      jobCards, tyres, mismatchCases, auditLogs, internalMessages, whatsAppMessages,
      tyreCompanies, customers, retreadOrders, complaints, payments, sapExchangePayload
    };
  ...
  }, [backendReady, jobCards, tyres, mismatchCases, auditLogs, internalMessages, whatsAppMessages, tyreCompanies, customers, retreadOrders, complaints, payments, sapExchangePayload]);
  ...
  useEffect(() => {
    fetch('/api/state')
      ...
      .then((state: Partial<{
        ...
        if (state.jobCards) setJobCards(state.jobCards);
        if (state.tyres) setTyres(state.tyres);
        if (state.mismatchCases) setMismatchCases(state.mismatchCases);
        if (state.auditLogs) setAuditLogs(state.auditLogs);
        if (state.internalMessages) setInternalMessages(state.internalMessages);
        if (state.whatsAppMessages) setWhatsAppMessages(state.whatsAppMessages);
        if (state.tyreCompanies) setTyreCompanies(state.tyreCompanies);
        if (state.customers) setCustomers(state.customers);
        if (state.retreadOrders) setRetreadOrders(state.retreadOrders);
        if (state.complaints) setComplaints(state.complaints);
        if (state.payments) setPayments(state.payments);
        if (state.sapExchangePayload) setSapExchangePayload(state.sapExchangePayload);
      })
      .catch(() => undefined)
  ...
  const syncOrderForJob = (jobCardId: string, status: RetreadOrder['status'], description: string) => {
    const job = jobCards.find((j) => j.id === jobCardId);
    if (!job?.orderNo) return;
    const timestamp = new Date().toISOString();
    setRetreadOrders((prev) =>
      prev.map((order) =>
        order.orderNo === job.orderNo
          ? {
              ...order,
              status,
              timeline: [...order.timeline, { status, timestamp, description }]
            }
          : order
      )
    );
  ...
  const createCustomer = (data: { name: string; address: string; pan: string; gst: string; mobileNo: string; fleetOrDealerType: 'Fleet Operator' | 'Dealer' | 'Direct Customer'; creditLimit?: number }): CustomerProfile => {
    if (!canOperate('SALES_EMPLOYEE')) return customers[0];
  ...
  const updateTyreFitnessClassification = (tyreId: string, fitness: FitnessClassification, remarks: string, ticked: boolean) => {
    if (!canOperate('INSPECTION_TEAM')) return;
    setTyres((prev) =>
      prev.map((t) =>
        t.id === tyreId
          ? {
  ...
  const advanceProductionStage = (jobCardId: string, stage: ProcessingStage, materialUsage?: MaterialUsage): boolean => {
    if (!canOperate('PRODUCTION_TEAM')) return false;
    const job = jobCards.find((j) => j.id === jobCardId);
    if (!job) return false;
    const jobTyres = tyres.filter((t) => t.jobCardId === jobCardId && t.status !== 'MISSING');
    const stageOrder: ProcessingStage[] = ['BUFFING', 'RASPING', 'REPAIR', 'COATING', 'CURING', 'PCI', 'PDI'];
    const currentIndex = job.processingStage ? stageOrder.indexOf(job.processingStage) : -1;
    const requestedIndex = stageOrder.indexOf(stage);
    const expectedIndex = currentIndex < 0 ? 0 : currentIndex + 1;
    const hasPendingInspection = jobTyres.some((t) => t.fitnessStatus === 'PENDING');
    if (hasPendingInspection || requestedIndex < 0 || (currentIndex >= 0 && requestedIndex !== currentIndex && requestedIndex !== expectedIndex)) return false;
    ...
    syncOrderForJob(jobCardId, 'PROCESSING', `Production stage ${stage} completed by M. Selvam.`);
  ...
  const submitQcApproval = (jobCardId: string, qcRecord: QCApprovalRecord): boolean => {
    if (!canOperate('PRODUCTION_TEAM', 'INSPECTION_TEAM')) return false;
    const job = jobCards.find((j) => j.id === jobCardId);
    const jobTyres = tyres.filter((t) => t.jobCardId === jobCardId && t.status !== 'MISSING');
    const hasOpenMismatch = mismatchCases.some((c) => c.jobCardId === jobCardId && c.status !== 'RESOLVED');
    if (!job || job.processingStage !== 'PDI' || jobTyres.length === 0 || jobTyres.some((t) => t.fitnessStatus === 'PENDING') || hasOpenMismatch) return false;
    ...
    setJobCards((prev) =>
      ...
      syncOrderForJob(jobCardId, qcRecord.approved ? 'READY_FOR_DISPATCH' : 'PROCESSING', qcRecord.approved ? 'QC approved; order ready for dispatch.' : 'QC rejected; order returned for rework.');
  ...
  const processPayment = (orderNo: string, amount: number, method: 'UPI' | 'CREDIT_CARD' | 'DEBIT_CARD' | 'RAZORPAY_GATEWAY'): PaymentTransaction => {
    if (!canOperate('ACCOUNTS', 'CUSTOMER')) throw new Error('Current role is not permitted to process payments.');
    const order = retreadOrders.find((o) => o.orderNo === orderNo);
    const customer = order ? customers.find((c) => c.id === order.customerId) : undefined;
    if (!order || amount <= 0 || amount > order.totalAmount - order.paidAmount) {
      throw new Error('Payment amount must be greater than zero and within the outstanding balance.');
    }
    ...
    if (customer) {
      setCustomers((prev) =>
        prev.map((c) => (c.id === customer.id ? { ...c, outstandingAmount: Math.max(0, c.outstandingAmount - amount) } : c))
      );
    }
    const paymentJob = jobCards.find((j) => j.orderNo === orderNo);
    if (paymentJob) syncOrderForJob(paymentJob.id, order.status, `Payment of ₹${amount.toLocaleString()} received via ${method}.`);
  ...
  const triggerSapB1Sync = (orderNo: string) => {
    ...
    const orderJob = jobCards.find((j) => j.orderNo === order.orderNo);
    const orderTyres = tyres.filter((t) => t.jobCardId === (orderJob?.id || selectedJobCardId));
  ...
  const dispatchOrder = (jobCardId: string): boolean => {
    if (!canOperate('ADMIN')) return false;
    const job = jobCards.find((j) => j.id === jobCardId);
    if (!job || job.status !== 'READY_FOR_DISPATCH' || !job.orderNo) return false;
    const timestamp = new Date().toISOString();
    setJobCards((prev) => prev.map((j) => j.id === jobCardId ? { ...j, status: 'DISPATCHED', updatedAt: timestamp } : j));
    setTyres((prev) => prev.map((t) => t.jobCardId === jobCardId ? { ...t, status: 'DISPATCHED', currentLocation: 'Dispatched to Customer' } : t));
    syncOrderForJob(jobCardId, 'DISPATCHED', 'Order dispatched to customer.');
    addAudit('Order Dispatched', 'Dispatch Team', 'Management', jobCardId, `Order ${job.orderNo} dispatched for delivery.`);
    return true;
  };

  const deliverOrder = (jobCardId: string): boolean => {
    if (!canOperate('ADMIN')) return false;
    const job = jobCards.find((j) => j.id === jobCardId);
    if (!job || job.status !== 'DISPATCHED' || !job.orderNo) return false;
    const timestamp = new Date().toISOString();
    setJobCards((prev) => prev.map((j) => j.id === jobCardId ? { ...j, status: 'DELIVERED', updatedAt: timestamp } : j));
    setTyres((prev) => prev.map((t) => t.jobCardId === jobCardId ? { ...t, status: 'DELIVERED', currentLocation: 'Delivered to Customer' } : t));
    syncOrderForJob(jobCardId, 'DELIVERED', 'Order delivered to customer and delivery confirmed.');
    addAudit('Order Delivered', 'Delivery Team', 'Management', jobCardId, `Order ${job.orderNo} delivered to ${job.customerName}.`);
    return true;
  };
  ...
  const registerDriverTyre = (jobCardId: string, tyreData: { serialNo: string; brand: string; make?: string; size: string; colorField?: string; dot?: string; casingCode?: string; type: 'Radial' | 'Bias'; condition: 'Retreadable' | 'Repair Needed' | 'Casing OK' | 'Damaged' }) => {
    ...
    const highestTyreNumber = tyres.reduce((highest, tyre) => {
      const number = Number(tyre.id.replace('TYR-', ''));
      return Number.isFinite(number) ? Math.max(highest, number) : highest;
    }, 0);
    const newTyreId = `TYR-${(highestTyreNumber + 1).toString().padStart(3, '0')}`;
  ...
  const verifyCustomerOtp = (jobCardId: string, inputOtp: string): boolean => {
    ...
    if (inputOtp.trim() === job.otpCode) {
      ...
      syncOrderForJob(jobCardId, 'TYRE_PICKED', `Pickup confirmed for ${confirmedCount} tyres by customer OTP.`);
  ...
  const startTransport = (jobCardId: string) => {
    const job = jobCards.find((j) => j.id === jobCardId);
    if (!job || !job.otpVerified || job.confirmedQty <= 0) return;
    setJobCards((prev) =>
      prev.map((j) =>
        j.id === jobCardId ? { ...j, status: 'IN_TRANSIT', updatedAt: new Date().toISOString() } : j
      )
    );
    syncOrderForJob(jobCardId, 'PICKUP_SCHEDULED', 'Driver started transport to the factory unit.');
    setTyres((prev) =>
  ...
  const handoverToGate = (jobCardId: string) => {
    const job = jobCards.find((j) => j.id === jobCardId);
    if (!job || job.status !== 'IN_TRANSIT' || !job.otpVerified) return;
    setJobCards((prev) =>
      prev.map((j) =>
        j.id === jobCardId ? { ...j, status: 'RECEIVING', updatedAt: new Date().toISOString() } : j
      )
    );
    syncOrderForJob(jobCardId, 'TYRE_PICKED', 'Driver handed the confirmed pickup to the receiving gate.');
  ...
  const completeGateReceiving = (jobCardId: string) => {
    ...
    if (received < expected) {
      ...
      const caseId = `MIS-${String(46 + mismatchCases.length).padStart(4, '0')}`;
      ...
      syncOrderForJob(jobCardId, 'RECEIVED_AT_UNIT', `Gate receiving completed with ${received} of ${expected} tyres; mismatch case ${caseId} opened.`);
      ...
      syncOrderForJob(jobCardId, 'RECEIVED_AT_UNIT', `All ${expected} tyres received and verified at the factory unit.`);
  ...
  const runGuidedScenarioStep = (step: number) => {
    ...
    if (step === 1) {
      ...
      setMgmtScreen(9);
      setSelectedCaseId(selectedCaseId || 'MIS-0045');
    } else if (step === 8) {
